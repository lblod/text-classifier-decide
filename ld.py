...
def write_airo_ai_model(hub_model_id: str, commit_info: CommitInfo, results: dict) -> Graph:
    ...
    g.add((
        model_uri,
        SD.dataPublished,
        Literal(datetime.datetime.now(tz=pytz.timezone("Europe/Brussels")).isoformat(), datatype=XSD.dateTime)
    ))
    ...
    for k, v in results.items():
        quality_metric_id = URIRef("http://example.com/qualitymeasurement/{0}".format(str(uuid.uuid4())))
    ...
    version_uri = URIRef("http://example.com/version/{0}".format(commit_info.oid))
    ...
    model_files_uri = URIRef("http://example.com/modelfiles/{0}".format(hub_model_id))
    ...
    code_uri = URIRef("http://example.com/code/{0}".format(repo.head.object.hexsha))
    ...
    source_code_uri = URIRef("http://example.com/sourcecode/lblod-text-classifier")
